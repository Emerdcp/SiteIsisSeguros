...
//================== BUSCAR SEGURADORAS ==================//
let seguradora = [];        // todos os dados vindos do fetch
...
function buscarSeguradoras() {
  fetch('../controller/cadastroSeguradora/buscar_seguradora.php')
    ...
    .then(data => {
      seguradora = data;
      indiceAtual = 0;
      document.getElementById('tabelaSeguradora').innerHTML = '';
      carregarMaisSeguradora(); // Carrega os primeiros 10
    })
    .catch(error => {
...
function carregarMaisSeguradora() {
  const tbody = document.getElementById('tabelaSeguradora');
  const proximoLote = seguradora.slice(indiceAtual, indiceAtual + porPagina);

  proximoLote.forEach(seguradora => {
  ...
  if (indiceAtual >= seguradora.length) {
...
document.getElementById('btnCarregarMais').addEventListener('click', carregarMaisSeguradora);
